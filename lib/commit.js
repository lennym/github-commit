...
class Commit {
  ...
  constructor({ repo, branch, token, auth }) {

    assert(repo, '`repo` is required');
    assert(auth || token, '`token` or `auth` is required');
    ...
    if (auth) {
      ...
      Object.defineProperty(this, 'auth', { value: token });
    }
  ...
  performGitAction (action, params = {}, scope = 'git') {
    ...
    const github = new Octokit({ auth: this.auth });
